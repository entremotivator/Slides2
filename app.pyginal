...
def get_gdrive_image_urls(folder_id: str):
    ...
    Uses advanced web scraping with multiple strategies to fetch ALL images.
    """
    images = []
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Strategy 1: Extract from embedded JSON data structures
            # Google Drive stores file info in various JavaScript variables
            json_patterns = [
                r'\["(https://lh3\.googleusercontent\.com/[^"]+)",\s*"[^"]*",\s*"[^"]*",\s*"([^"]+)"\]',
                r'\["([a-zA-Z0-9_-]{25,})",\s*"([^"]+)",.*?image',
            ]
            
            # Strategy 2: Look for _DRIVE_ivd data
            drive_data_pattern = r'window\[\'_DRIVE_ivd\'\]\s*=\s*\'(.+?)\';'
            data_match = re.search(drive_data_pattern, html_content)
            ...
            if data_match:
                try:
                    ...
                    # Unescape the string
                    data_str = data_str.replace('\\x', '%').replace('\\\\', '\\')
                    
                    # Find all potential file IDs (Google Drive file IDs are 25-44 chars)
                    potential_ids = re.findall(r'\["([a-zA-Z0-9_-]{25,44})"', data_str)
                    
                    for file_id in potential_ids:
                        if file_id != folder_id and file_id not in file_ids_found:
                            file_ids_found.add(file_id)
                except Exception as e:
                    st.warning(f"Data extraction error: {str(e)}")
            
            # Strategy 3: Direct file ID regex search (most comprehensive)
            # Look for patterns that match Google Drive file IDs
            all_file_ids = re.findall(r'["\'\\]([a-zA-Z0-9_-]{28,44})["\']', html_content)
            
            for file_id in all_file_ids:
                if file_id != folder_id and file_id not in file_ids_found and len(file_id) >= 25:
                    file_ids_found.add(file_id)
            
            # Strategy 4: Look for specific image markers in HTML
            # Sometimes Google embeds image references with "image/" in nearby text
            image_sections = re.findall(r'image/[a-z]+[^{]+?([a-zA-Z0-9_-]{28,44})', html_content, re.IGNORECASE)
            for file_id in image_sections:
                if file_id != folder_id and file_id not in file_ids_found:
                    file_ids_found.add(file_id)
            
            # Convert all found IDs to image metadata
            ...
            for idx, file_id in enumerate(file_ids_found):
                progress_bar.progress((idx + 1) / total_ids)
                status_text.text(f"Processing {idx + 1}/{total_ids} files...")
                
                # Try to detect if this is actually an image by doing a HEAD request
                try:
                    ...
                    # Only add if it's confirmed to be an image
                    if 'image' in content_type.lower():
                        images.append({
                            "name": f"Image_{valid_count + 1}_{file_id[:8]}.jpg",
                            "url": test_url,
                            "source": "gdrive",
                            "file_id": file_id,
                        ...
                        valid_count += 1
                    # If no content type check works, add it anyway (we'll verify on load)
                    elif valid_count < 50:  # Limit unverified additions
                        images.append({
                            "name": f"Image_{valid_count + 1}_{file_id[:8]}.jpg",
                            "url": test_url,
                            "source": "gdrive",
                    ...
                    # Add without verification if network fails
                    if valid_count < 100:
                        images.append({
                            "name": f"Image_{valid_count + 1}_{file_id[:8]}.jpg",
            ...
            if images:
                ...
                st.info(f"📊 Scanned {total_ids} total files, identified {len(images)} as images")
            else:
            ...
            st.error(f"❌ Could not access folder (Status: {response.status_code})")
        ...
        st.info("Try again or use a smaller folder")
        return []
    except Exception as e:
